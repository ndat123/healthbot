...
@section('content')
    <div class="container mx-auto px-4 py-8">
        <div class="flex justify-between items-center mb-8">
            ...
            <a href="#" onclick="alert('Coming Soon!')" class="bg-primary text-white px-4 py-2 rounded-md hover:bg-blue-700 transition">Schedule New Consultation</a>
        </div>
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @foreach($consultations as $consultation)
                            <tr>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{{ $consultation['user'] }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $consultation['specialist'] }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $consultation['date'] }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $consultation['time'] }}</td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full {{ $consultation['status'] === 'Scheduled' ? 'bg-blue-100 text-blue-800' : 'bg-green-100 text-green-800' }}">
                                        {{ $consultation['status'] }}
                                    </span>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $consultation['type'] }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                    <a href="#" onclick="alert('Coming Soon!')" class="text-primary hover:text-blue-900 mr-3">View</a>
