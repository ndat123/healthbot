...
<div class="container mx-auto px-4 py-8">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        ...
        <div class="bg-white rounded-2xl shadow-xl p-6 border border-gray-100">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-gray-500 text-sm font-medium">Doctors</p>
    ...
    <div class="bg-white rounded-2xl shadow-xl p-6 border border-gray-100 mb-8">
        ...
        <form method="GET" action="{{ route('admin.users') }}" class="mb-6">
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div>
                    <input type="text" name="search" value="{{ $filters['search'] ?? '' }}" placeholder="Search by name or email..." class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                </div>
                <div>
                    ...
                    @forelse($users as $user)
                        <tr class="hover:bg-gray-50 transition duration-150">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium flex space-x-2">
                                <button onclick="viewUser({{ $user['id'] }})" class="text-blue-600 hover:text-blue-900" title="View Profile">
                                ...
                                <form action="{{ route('admin.users.toggle-lock', $user['id']) }}" method="POST" class="inline">
                                    ...
                                    @if($user['status'] == 'Active')
                                        <button type="submit" class="text-yellow-600 hover:text-yellow-900" title="Lock Account">
                                            <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"/>
                                            </svg>
                                        </button>
                                    @else
                                        <button type="submit" class="text-green-600 hover:text-green-900" title="Unlock Account">
                                            <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div class="mt-6 flex justify-between items-center">
            <div class="text-sm text-gray-700">
                Showing <span class="font-medium">{{ $pagination->firstItem() }}</span> to <span class="font-medium">{{ $pagination->lastItem() }}</span> of <span class="font-medium">{{ $pagination->total() }}</span> results
            </div>
