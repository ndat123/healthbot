...
<div class="container mx-auto px-4 py-8">
    ...
    <div class="bg-white rounded-2xl shadow-xl p-6 border border-gray-100">
        <form action="{{ route('admin.medical-content.faq.update', $faq->id) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <textarea id="answer" 
                          ...
                          rows="8"
                          required
                          class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent @error('answer') border-red-500 @enderror">{{ old('answer', $faq->content) }}</textarea>
                @error('answer')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
